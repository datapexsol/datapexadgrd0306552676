...
public class client
{
           ...
           string inFavorOf, string jsAccount, string cashOrderNo, string amount, string amountInWords)
    {
        ...
        if (checkClientExist == false)
        {
            ...
            cmd = new SqlCommand("insert into admin_registeration.dbo.client_info values(@name,@relation_of,@applicant_cnic,@occupation,@present_address,@telephone,@mobile,@email,@permanent_address,@nominee_name,@nominee_address,@nominee_cnic,@nominee_no,@image,@registeration_no)", con);
            ...
            //payment
            cmd = new SqlCommand("insert into admin_registeration.dbo.payment values( @amountin_rs, @date,@client_id, @propertyid,@amount_in_words,@type,@payment_made_through,@infavour_of,@jsbankaccount,@cash_payorder_no)", con);
    ...
    public static DataTable viewClients(string cnic)
    {
        ...
        conn.Open();
        SqlDataAdapter da = new SqlDataAdapter("select * from admin_registeration.dbo.client_info where admin_registeration.dbo.client_info.applicant_cnic='"+cnic+"' OR registeration_no='"+cnic+"'", conn);
        DataSet ds = new DataSet();
        da.Fill(ds, "client_info");
        DataTable dt = ds.Tables["client_info"];
        int count = dt.Rows.Count;
