...
public class applicant_info
{
    ...
    public static string downpayment(String name, string relation, string cnic, string occupation, string presentAddress, string telephone, string mobile, string email, string permanent_address, string nomineeName, string nomineeAddress,
       string nomineeCnic, string no, byte[] image, string regno,string amount , string amount_words,string plotsize,string cashmadethrough,string plotno,string street,string plotaddress,string propertytype)
    {
        ...
        if (checkInstallmentPlan(plotsize) == true)
        {
            ...
            {
                SqlCommand cmd1 = new SqlCommand("UPDATE admin_registeration.dbo.client_info SET  mobile = '" + mobile + "', email = '" + email + "', permanent_address = '" + permanent_address + "', nominee_name = '" + nomineeName + "', nominee_address = '" + nomineeAddress + "', nominee_cnic = '" + nomineeCnic + "', nominee_no = '" + no + "', image = @image where Id = '" + clientId + "'", conn);
            ...
            if (checkplotno(int.Parse(plotno)) == true && clientregnoAvailbilty(regno) == true)
            {
                ...
                cmd = new SqlCommand("insert into admin_registeration.dbo.payment values( @amountin_rs, @date,@client_id, @propertyid,@amount_in_words,@type,@payment_made_through,@infavour_of,@jsbankaccount,@cash_payorder_no,@property_registration)", conn);
   ...
   public static int getClientId(string cnic)
    {
        SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString);
        conn.Open();
        //SqlDataAdapter da = new SqlDataAdapter("select * from admin_registeration.dbo.client_info  order by id DESC", conn);
        string query = "select Id from admin_registeration.dbo.client_info where applicant_cnic='"+cnic+"' order by id DESC";
    ...
    public static int getpropertyid(int plotno)
    {
        SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString);
        conn.Open();
        // SqlDataAdapter da = new SqlDataAdapter("select * from admin_registeration.dbo.property_info  order by id DESC", conn);
        string query = "select id from admin_registeration.dbo.property_info where plot_no='"+plotno+"'";
        SqlCommand sql=  new SqlCommand(query, conn);
        int temp = Convert.ToInt32(sql.ExecuteScalar().ToString());
    ...
    {
        SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString);
        String checkUser = "select count(*) from admin_registeration.dbo.property_info where plot_no='" +plotno+ "'";
    ...
    {
        SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString);
        String checkUser = "select count(*) from admin_registeration.dbo.client_info where applicant_cnic='" + userinput + "'";
    ...
    {
        SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString);
        String checkUser = "select count(*) from admin_registeration.dbo.client_info where applicant_cnic='"+cnic+"' AND email='' AND nominee_name='' AND nominee_cnic=''";
...
inner join admin_registeration.dbo.property_info on admin_registeration.dbo.client_info.id = admin_registeration.dbo.property_info.client_id
 where admin_registeration.dbo.property_info.registrationo = '"+regno+"' AND admin_registeration.dbo.property_info.plot_no=''; ", conn);
        int temp = Convert.ToInt32(cmd.ExecuteScalar().ToString());
        //  cmd.BeginExecuteNonQuery();
    ...
    public static void updatePaymentMemberShip(int pid, string regno)
    {
        ...
        conn.Open();
        SqlCommand cmd = new SqlCommand(@"update admin_registeration.dbo.payment set admin_registeration.dbo.payment.property_id=@pid where admin_registeration.dbo.payment.property_registration='"+regno+"' And admin_registeration.dbo.payment.payment_type='membership'", conn);
