...
public class installmentplan
{
    ...
    public static DataTable view()
    {
        ...
        SqlCommand cmd = new SqlCommand("SELECT * FROM admin_registeration.dbo.installments_plan_info", conn);
        conn.Open();
    ...
    public static bool insertInstallments(string tbinstallmentcost, string tbFirstName, string tbplotsize, string tbdownpayment, string tblumsum, string tbsize, string tbtotalcost, string tbmembershipfee)
    {
        ...
        cmd.CommandText = "INSERT INTO admin_registeration.dbo.installments_plan_info ( installment_cost_per_month,installments_duration, plot_size,down_payment,lum_sum_discount,Marla,totalcost,membership_fee ) VALUES ( @tbinstallmentcost, @FirstName,@plotsize,@downpayment,@lumsum,@Marla,@totalcost,@membershipfee )";
    ...
    public static bool updateInstallments(string strPersonID, string strLastName, string strFirstName, string duration, string downpayment, string lumsum, string size, string totalCost , string membershipfee)
    {
        ...
        // Set the command text
        // SQL statement or the name of the stored procedure 
        cmd.CommandText = "UPDATE admin_registeration.dbo.installments_plan_info SET installment_cost_per_month = @LastName, plot_size = @FirstName , installments_duration=@duration,down_payment=@down_payment, lum_sum_discount=@lum_sum_discount, Marla=@Size, totalcost=@totalcost,membership_fee=@membershipfee WHERE id = @PersonID";
    ...
    public static bool deleteInstallments(string strPersonID)
    {
        using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString()))
        {
            ...
            // Set the command text
            // SQL statement or the name of the stored procedure 
            cmd.CommandText = "DELETE FROM admin_registeration.dbo.installments_plan_info WHERE id = @PersonID";
